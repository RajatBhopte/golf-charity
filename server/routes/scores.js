...
router.post("/", requireActiveSubscription, async (req, res) => {
  ...
  if (!score || !played_date) {
    return res
      .status(400)
      .json({ error: "Score and played date are required" });
  }
  ...
  if (playedDateObj > today) {
    return res
      .status(400)
      .json({ error: "Played date cannot be in the future" });
  }
  if (playedDateObj < oneYearAgo) {
    return res
      .status(400)
      .json({ error: "Played date cannot be more than 1 year old" });
  }
  ...
  try {
    ...
    if (score !== undefined) {
      ...
      if (isNaN(numericScore) || numericScore < 1 || numericScore > 45) {
        return res
          .status(400)
          .json({ error: "Score must be between 1 and 45" });
      }

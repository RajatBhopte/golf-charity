...
The project uses these primary tables in `public` schema:

- `users`: user profile, role, subscription, selected charity.
- `scores`: player score entries (rolling score workflow).
- `charities`: charity catalog, spotlight, and totals.
- `draws`: monthly draw metadata, pool split, winning numbers, rollover.
- `winners`: draw winners, matched numbers, verification and payment status.
- `notifications`: user notifications for draw/winner updates.
- `winner_audit_logs`: admin audit trail for winner state changes.
- `donations`: direct user donations to charities (from PRD gap migration).

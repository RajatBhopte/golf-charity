  ...
  const filteredWinners = useMemo(
    () =>
      winners.filter((winner) => {
        ...
        if (filter === "pending")
          return matchesSearch && winner.verification_status === "pending";
        if (filter === "approved")
          return (
            matchesSearch &&
        ...
        if (filter === "paid")
          return matchesSearch && winner.payment_status === "paid";
                ...
                <td className="px-6 py-4">
                  <div>
                    <div className="font-bold flex items-center gap-2">
                      ...
                      {w.proof_signed_url ? (
                        <a
                          href={w.proof_signed_url}
                          target="_blank"
                          rel="noreferrer"
                          className="text-brand-500 hover:text-brand-400"
                          title="Open proof screenshot"
                        >
                          <Eye size={14} />
                        </a>
                      ) : (
                    ...
                    <div className="text-xs text-gray-500">
                      Draw:{" "}
                      {w.draws?.month_year
                        ? new Date(w.draws.month_year).toLocaleDateString([], {
                            month: "long",
                            year: "numeric",
                          })
                        : "Unknown draw"}
                    ...
                    <div className="text-xs text-gray-500">
                      Winning Numbers:{" "}
                      {formatNumberSeries(w.draws?.winning_numbers)}
                    </div>
                    <div className="text-xs text-gray-500">
                    ...
                    {Array.isArray(w.audit_logs) && w.audit_logs.length > 0 && (
                      <div
                        className={`mt-3 rounded-xl border px-3 py-2 text-xs ${isDark ? "border-dark-border bg-dark-bg/70 text-gray-300" : "border-light-border bg-gray-50 text-light-text"}`}
                      >
                        <div className="mb-2 font-bold uppercase tracking-wider text-brand-500">
                          Audit Trail
                        </div>
                  ...
                  <div className="flex justify-center">
                    {(() => {
                      ...
                      const canProcessPayment =
                        w.verification_status === "approved" && !isPaid;
                      const isUpdating = updatingId === w.id;
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            updateStatus(w.id, { payment_status: "paid" })
                          }

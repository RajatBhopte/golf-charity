...
export default function ScoreList({ scores = [], onEdit, onDelete, loading }) {
  ...
  return (
    <div
      className={`flex flex-col h-full rounded-2xl border backdrop-blur-xl ${
        isDark
    ...
    >
      <div className="p-6 sm:p-8 border-b border-opacity-50 border-inherit">
        ...
        <p
          className={`text-sm mt-1 ${isDark ? "text-gray-400" : "text-light-subtext"}`}
        >
      ...
      <div className="p-6 sm:p-8 flex-1 flex flex-col gap-3">
        {loading && scores.length === 0 ? (
          ...
          <div
            className={`flex flex-col items-center justify-center text-center py-10 rounded-xl border border-dashed ${isDark ? "border-dark-border bg-dark-bg/50" : "border-light-border bg-gray-50"}`}
          >
            ...
            <p
              className={`text-sm max-w-[200px] mx-auto ${isDark ? "text-gray-400" : "text-light-subtext"}`}
            >
              ...
              return (
                <div
                  ...
                  className={`group relative flex items-center justify-between p-4 rounded-xl border transition-all ${
                    isEditing
                      ? isDark
                        ? "bg-dark-bg border-brand-500/50"
                        : "bg-brand-50 border-brand-500/50"
                      : isDark
                        ? "bg-dark-bg border-dark-border hover:border-brand-500/30"
                        : "bg-gray-50 border-light-border hover:border-brand-500/30"
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-3 w-full">
                      ...
                      <input
                        ...
                        className={`score-date-input flex-1 min-w-0 pl-2 pr-10 py-1 text-xs sm:text-sm rounded-lg border outline-none ${isDark ? "bg-dark-card border-dark-border [color-scheme:dark]" : "bg-white border-gray-300 [color-scheme:light]"}`}
                      />
                    ...
                    <>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-semibold text-sm">
                            {new Date(scoreObj.played_date).toLocaleDateString(
                              undefined,
                              {
                                weekday: "short",
                                month: "short",
                      ...
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                        <button
                          onClick={() => handleEditClick(scoreObj)}
                          disabled={actionLoading}
                          className={`p-2 rounded-lg transition-colors ${isDark ? "hover:bg-dark-card text-gray-400 hover:text-white" : "hover:bg-white text-gray-500 hover:text-gray-900"}`}
                        >
                          <Pencil size={16} />
                        </button>
                        <button
                          onClick={() => handleDelete(scoreObj.id)}
                          disabled={actionLoading}
                          className={`p-2 rounded-lg transition-colors ${isDark ? "hover:bg-dark-card text-gray-400 hover:text-red-500" : "hover:bg-white text-gray-500 hover:text-red-500"}`}
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
      ...
      <div
        className={`p-4 sm:p-6 mt-auto border-t rounded-b-2xl ${
          scores.length === 5
            ? isDark
        ...
        <div className="flex items-center gap-3">
          {scores.length === 5 ? (
            ...
            <>
              ...
              >
                ⚠️ Add {5 - scores.length} more score
                {scores.length === 4 ? "" : "s"} to enter the draw!
              </p>
            </>
          )}
        </div>

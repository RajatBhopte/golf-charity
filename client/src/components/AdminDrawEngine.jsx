...
import { Trophy, Settings2, Play, Send, Loader2, Info, AlertTriangle, CreditCard } from 'lucide-react';
...
export default function AdminDrawEngine({ isDark }) {
  ...
  const runSimulation = async () => {
    ...
    if (!monthStart) {
      setError('Please choose a valid draw month before running the simulation.');
      setSimulation(null);
      return;
    }
    ...
    try {
      ...
      if (!data.eligible_participants) {
        setNotice('No eligible participants have logged five scores for this draw month yet.');
      }
    } catch (error) {
  ...
  const publishDraw = async () => {
    if (!simulation || !session || !confirm('Are you sure you want to publish these results officialy? This will notify winners and lock the draw.')) return;
    const monthStart = toMonthStart(config.month_year);
    if (!monthStart) {
    ...
    try {
      ...
      setNotice(`Draw published successfully with ${data.winner_count || 0} recorded winners.`);
      setSimulation(null);
    } catch (error) {
      setError(error.message);
    } finally {
      ...
      <div className={`glass-card p-6 rounded-2xl border ${isDark ? 'bg-dark-card border-dark-border' : 'bg-white border-light-border shadow-sm'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="space-y-4">
            <label className="block text-sm font-bold uppercase tracking-wider opacity-50">Draw Month</label>
            <input
              type="month"
          ...
          <div className="space-y-4">
            <label className="block text-sm font-bold uppercase tracking-wider opacity-50">Draw Methodology</label>
            <div className="flex flex-col gap-2">
              ...
              >
                <div className="text-left">
                  ...
                  <div className="text-xs opacity-60">Equal chance for all eligible users</div>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${config.type === 'random' ? 'border-brand-500' : 'border-gray-500'}`}>
                  {config.type === 'random' && <div className="w-2.5 h-2.5 rounded-full bg-brand-500" />}
                </div>
              </button>
              <button 
              ...
              >
                <div className="text-left">
                  ...
                  <div className="text-xs opacity-60">Favors frequency or consistency</div>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${config.type === 'algorithmic' ? 'border-brand-500' : 'border-gray-500'}`}>
                  {config.type === 'algorithmic' && <div className="w-2.5 h-2.5 rounded-full bg-brand-500" />}
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <label className="block text-sm font-bold uppercase tracking-wider opacity-50">Algorithmic Weighting</label>
            <div className={`flex flex-col gap-2 ${config.type !== 'algorithmic' ? 'opacity-30 pointer-events-none' : ''}`}>
              <button 
              ...
              >
                <div className="text-left">
                  ...
                  <div className="text-xs opacity-60">Reward active users (More scores = More entries)</div>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${config.weighting === 'most' ? 'border-blue-500' : 'border-gray-500'}`}>
                  {config.weighting === 'most' && <div className="w-2.5 h-2.5 rounded-full bg-blue-500" />}
                </div>
              </button>
              <button 
              ...
              >
                <div className="text-left">
                  <div className="font-bold text-amber-500">Least Frequent</div>
                  <div className="text-xs opacity-60">Anti-monopoly boost (Newer/Casual players get a tailwind)</div>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${config.weighting === 'least' ? 'border-amber-500' : 'border-gray-500'}`}>
                  {config.weighting === 'least' && <div className="w-2.5 h-2.5 rounded-full bg-amber-500" />}
                </div>
              </button>
            </div>
          </div>
      ...
      {simulation && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className={`glass-card p-6 rounded-2xl border ${isDark ? 'bg-dark-card border-dark-border' : 'bg-white border-light-border shadow-sm'}`}>
              <h3 className="font-bold mb-4 flex items-center gap-2"><Info size={18} className="text-blue-500" /> Draw Summary</h3>

...
const SUBSCRIPTION_OPTIONS = ['active', 'pending', 'inactive', 'suspended', 'cancelled'];
...
export default function AdminUserList({ isDark }) {
  ...
  const filteredUsers = useMemo(() => users.filter((user) => {
    ...
    const matchesPlan = planFilter === 'all' || (user.subscription_plan || 'monthly') === planFilter;
    const matchesStatus = statusFilter === 'all' || user.subscription_status === statusFilter;
    const matchesCharity = charityFilter === 'all' || (user.charity_id || '') === charityFilter;

    return matchesSearch && matchesRole && matchesPlan && matchesStatus && matchesCharity;
  ...
  const saveUser = async (event) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/admin/users/${editingUser.id}`), {
        method: 'PUT',
        headers: authHeaders,
        body: JSON.stringify({
          ...userForm,
  ...
  const openScoresModal = async (user) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/admin/users/${user.id}/scores`), {
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
      const payload = await response.json();
      if (!response.ok) {
  ...
  const addScore = async (event) => {
    ...
    setScoreError('');
    try {
      const response = await fetch(buildApiUrl(`/admin/users/${activeScoresUser.id}/scores`), {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({
  ...
  const saveScore = async (score) => {
    ...
    setScoreError('');
    try {
      const response = await fetch(buildApiUrl(`/admin/users/${activeScoresUser.id}/scores/${score.id}`), {
        method: 'PUT',
        headers: authHeaders,
        body: JSON.stringify({
      ...
      setScores((currentScores) => currentScores.map((entry) => (
        entry.id === score.id ? payload : entry
      )));
    } catch (error) {
      setScoreError(error.message);
    } finally {
  ...
  const deleteScore = async (scoreId) => {
    ...
    setScoreError('');
    try {
      const response = await fetch(buildApiUrl(`/admin/users/${activeScoresUser.id}/scores/${scoreId}`), {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
  ...
  if (loading) {
    return <div className="flex justify-center py-20"><Loader2 className="animate-spin text-brand-500" size={40} /></div>;
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className={isDark ? 'bg-white/5' : 'bg-gray-50'}>
              ...
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-500">Subscription</th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-500">Charity</th>
          ...
          <tbody className={isDark ? 'divide-y divide-dark-border' : 'divide-y divide-light-border bg-white'}>
            {filteredUsers.map((user) => (
              <tr key={user.id} className={`${isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'} transition-colors`}>
                <td className="px-6 py-4">
                  <div className="font-bold">{user.full_name || 'Anonymous Golfer'}</div>
                  <div className="text-sm text-gray-500">{user.email || 'No email on file'}</div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-sm">
                  <div className="font-semibold">{charityMap.get(user.charity_id) || 'Unassigned'}</div>
                  <div className="text-gray-500">{user.charity_percentage ?? 0}% allocation</div>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      {updatingRoleId === user.id ? <Loader2 className="animate-spin" size={16} /> : <UserCog size={16} />}
                    </ActionButton>
                    <ActionButton
                      isDark={isDark}
                      title="Edit profile"
      ...
      {editingUser && (
        <ModalShell title={`Edit ${editingUser.full_name || editingUser.email || 'User'}`} isDark={isDark} onClose={closeEditModal}>
          <form onSubmit={saveUser} className="space-y-4">
            <TextField label="Full Name" value={userForm.full_name} onChange={(value) => setUserForm((current) => ({ ...current, full_name: value }))} isDark={isDark} />
            <TextField label="Email" type="email" value={userForm.email} onChange={(value) => setUserForm((current) => ({ ...current, email: value }))} isDark={isDark} />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <SelectField
                ...
                onChange={(value) => setUserForm((current) => ({ ...current, subscription_plan: value }))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <SelectField
                ...
                onChange={(value) => setUserForm((current) => ({ ...current, subscription_status: value }))}
                options={SUBSCRIPTION_OPTIONS.map((status) => ({ value: status, label: status }))}
                isDark={isDark}
              />
              <TextField
                label="Charity %"
                ...
                onChange={(value) => setUserForm((current) => ({ ...current, charity_percentage: value }))}
                isDark={isDark}
              />
              <SelectField
                label="Assigned Charity"
                value={userForm.charity_id}
                onChange={(value) => setUserForm((current) => ({ ...current, charity_id: value }))}
                options={[{ value: '', label: 'No charity selected' }, ...charities.map((charity) => ({ value: charity.id, label: charity.name }))]}
                isDark={isDark}
              />
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={closeEditModal} className="px-4 py-2 rounded-xl border border-white/10 text-sm font-semibold">
                Cancel
              </button>
              <button type="submit" disabled={savingUser} className="btn-primary !py-2.5 flex items-center gap-2">
      ...
      {activeScoresUser && (
        <ModalShell title={`Manage Scores: ${activeScoresUser.full_name || activeScoresUser.email || 'User'}`} isDark={isDark} onClose={closeScoresModal} widthClass="max-w-4xl">
          <div className="space-y-6">
            <form onSubmit={addScore} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-3">
              <TextField
                label="Score"
                type="number"
                value={String(scoreForm.score)}
                onChange={(value) => setScoreForm((current) => ({ ...current, score: value }))}
                isDark={isDark}
              />
              <TextField
                label="Played Date"
                ...
                onChange={(value) => setScoreForm((current) => ({ ...current, played_date: value }))}
                isDark={isDark}
              />
              <button type="submit" disabled={savingScoreId === 'new'} className="btn-primary !py-3 mt-auto flex items-center justify-center gap-2">
                {savingScoreId === 'new' ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}
                Add Score
              </button>
            </form>
            ...
            {scoresLoading ? (
              ...
              <div className={`overflow-hidden rounded-2xl border ${isDark ? 'border-dark-border' : 'border-light-border'}`}>
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className={isDark ? 'bg-white/5' : 'bg-gray-50'}>
                      <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-gray-500">Score</th>
                      <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-gray-500">Played Date</th>
                      <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-gray-500 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className={isDark ? 'divide-y divide-dark-border' : 'divide-y divide-light-border bg-white'}>
                    {scores.map((score) => (
                      <tr key={score.id}>
                        <td className="px-4 py-3">
                          <input
                            ...
                            max="45"
                            value={score.score}
                            onChange={(event) => updateScoreField(score.id, 'score', event.target.value)}
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            onChange={(event) => updateScoreField(score.id, 'played_date', event.target.value)}
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            ...
                            >
                              {savingScoreId === score.id ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}
                            </ActionButton>
                            <ActionButton
                              isDark={isDark}
                              title="Delete score"
...
function SelectFilter({ icon, value, onChange, options, isDark }) {
  ...
  return (
    <div className="relative">
      <SelectIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
      <select
        value={value}
  ...
  return (
    <button
      className={`p-2 rounded-lg transition-all ${
        isDark ? 'hover:bg-white/10 text-gray-300' : 'hover:bg-gray-100 text-light-subtext'
      } disabled:opacity-50`}
      {...props}
    >
      {children}
...
function ModalShell({ title, children, isDark, onClose, widthClass = 'max-w-2xl' }) {
  return (
    <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm p-4 flex items-center justify-center">
      <div className={`w-full ${widthClass} rounded-3xl border p-6 ${isDark ? 'bg-dark-bg border-dark-border' : 'bg-white border-light-border'}`}>
...
function TextField({ label, value, onChange, isDark, type = 'text' }) {
  return (
    <label className="block">
      <span className="block text-sm font-medium mb-1.5 opacity-70">{label}</span>
      <input
        type={type}
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className={`w-full px-4 py-3 rounded-xl border ${
          isDark ? 'bg-dark-card border-dark-border text-white' : 'bg-gray-50 border-light-border'
        }`}
      />
    </label>
  );
